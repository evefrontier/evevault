...
export const ephKeyService = {
  ...
  async getEphemeralPublicKey(): Promise<Ed25519PublicKey | null> {
    ...
    if (res?.ok && res.publicKeyBytes) {
      ...
      const publicKey = new Ed25519PublicKey(new Uint8Array(res.publicKeyBytes))
      return publicKey
    } else {
 ...
 * The keypair lives in keeper RAM; the encrypted private key is persisted under
 * the device store for dev convenience across restarts. This is acceptable for
